...
import { login as technicianLogin } from "store/authPartnerSlice";
import { toastMessage } from "utils/utility";

import { postData } from "services/api";
import FormControlLabel from "@mui/material/FormControlLabel";
import Checkbox from "@mui/material/Checkbox";
const steps = ["Next", "Login", "Verify OTP", "Confirm Detail"];
...
function Login() {
  ...
  const sendOtp = async () => {
    ...
    if (mobile.length == 10) {
      ...
      try {
        let res = await postData("partner/check", { phone: mobile });
        if (res.status === 2) {
          handelStep(1);
        } else {
          handelStep(2);
  ...
  const loginPartner = async () => {
    if (password && userInfo["mobile"].value) {
      try {
        let res = await postData("partner/login", {
          phone: userInfo["mobile"].value,
          password: password
        });

        if (res.status === 1) {
          if (res.technician !== undefined) {
            dispatch(technicianLogin(res.technician));
            Router.push("/partner");
          } else {
            handelStep();
          }
  ...
  const validateOtp = async () => {
    ...
    if (otp && userInfo["mobile"].value && otp.length == 4) {
      try {
        let res = await postData("partner/otp/validate", {
          phone: userInfo["mobile"].value,
          otp: otp
        });
